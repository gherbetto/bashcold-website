$(function (){
	...
	if ($('.main-production-slider').length > 0) {
		$('.main-production-slider').slick({
			...
			pauseOnHover: false,
			infinite: true,
			prevArrow:"<button type='button' class='slick-prev'><i class='mps-arrow-left'></i></button>",
            nextArrow:"<button type='button' class='slick-next'><i class='mps-arrow-right'></i></button>"
		});
	}
	...
	$(document).on('click', function(event){
		if(( $(event.target).closest(".big-drp").length ))
		return;
	...
	$(document).on('click', function(event){
		if(( $(event.target).closest(".catalog-mob-select").length ))
		return;
